...
\chapter{System weryfikacji jakości kodu}
...
Podobieństw do idei proponowanych przez Ruby on Rails jest więcej. Wszelkie ustawienia aplikacji umieszczone są w specjalnych plikach konfiguracyjnych. Zdefiniowane w nich biblioteki i zależności pobierane są automatycznie. Samo wygenerowanie szkieletu sprowadza się do wpisania jednego polecenia. W przypadku Play jest to \texttt{play new nazwaAplikacji}.

Podczas tworzenia aplikacji możliwe jest uruchomienie jej w środowisku deweloperskim i utrzymywanie~w~stanie działania. Wszystkie wprowadzane do kodu zmiany są na bieżąco aktualizowane i widoczne. Jest to zdecydowana oszczędność czasu, gdyż nie ma potrzeby przebudowywania aplikacji i uruchamiania ponownie serwera. Innym atutem z tym związanym jest fakt, że gdy dojdzie do pojawienia się błędów kompilacji kodu, to zostaną one wyświetlone przez przeglądarkę. Wyszczególniony zostaje również plik oraz numer linii, w której do błędu doszło.

Warto też dodać, że Play Framework posiada wtyczki do popularnych zintegrowanych środowisk programistycznych jak IntelliJ IDEA, czy Eclipse. Obie aplikacje oraz szablon projektu SBT udostępniany przez część zarządzającą systemu mają dołączone zależności umożliwiające wygenerowanie danych konfiguracyjnych niezbędnych do zaimportowania projektu do tych narzędzi.

\subsection{MongoDB}

MongoDB to wydajna baza przechowująca dane w postaci dokumentów. Jeden serwer MongoDB może obsługiwać wiele baz danych. Pojedyńcza baza zawiera kolekcje, które są zbiorami dokumentów. Dokumenty mogą być dynamicznie modyfikowane i nie są ograniczone przez żaden schemat nakładany na kolekcje. Oznacza to w praktyce, że dokumenty w obrębie kolekcji mogą mieć różne zestawy pól.

Oficjalny sterownik dla języka Scala nosi nazwę \emph{Casbah}. Jest to tak naprawdę ,,opakowanie'' sterownika dla Javy. Społeczność Scali wytworzyła również wiele dodatkowych narzędzi do współpracy z MongoDB. Jedno z najbardziej popularnych to biblioteka \emph{Salat} służąca do przekształcania klas definiujących modele aplikacji do dokumentów MongoDB i odwrotnie. Udostępnia również pomocnicze metody, pozwalające w wygodny sposób skonstruować zapytania do bazy. System weryfikacji kodu Scala korzysta z zarówno z Casbah jak i Salat.

MongoDB oferuje również mechanizm pozwalający na przechowywanie plików w bazie, o nazwie \emph{GridFS}. Pliki umieszczane w bazie przy użyciu tej techniki są dzielone na mniejsze fragmenty i przechowywane jako osobne dokumenty. Po dodaniu pierwszego pliku, tworzone są dwie kolekcje. Jedna z nich przechowuje fragmenty plików, a druga -- metadane na ich temat. Są to między innymi nazwa, rozmiar oraz typ pliku. Sterownik automatycznie odnajduje potrzebne fragmenty pliku i łączy je w całość. 

\subsection{RabbitMQ}

Wymiana informacji na temat zadań i ich rozwiązań pomiędzy aplikacjami wchodzącymi w skład systemu, oparta jest na protokole AMQP\footnote{AMQP - jest to skrót od Advanced Message Queuing Protocol}. Pozwala on na asynchroniczne przekazywanie wiadomości oraz ogólne zarządzanie komunikacją wykorzystując do tego celu mechanizm kolejkowania.
...
W konfiguracji aplikacji zarządzającej wymagane jest zdefiniowanie dwóch pól -- nazwy maszyny, na której działa serwer RabbitMQ oraz nazwy kolejki służącej do obsługi rozwiązań.

\begin{alltt}
rabbitmq.host=\textcolor{OliveGreen}{localhost}
rabbitmq.solutions.queue=\textcolor{OliveGreen}{solutions_queue}
\end{alltt}

Aplikacja testująca kod wykorzystuje dodatkowe pole konfiguracyjne określające liczbę uruchamianych aktorów analizujących rozwiązania.  Aby uniknąć problemów wydajnościowych, jego wartość powinna uwzględniać konfigurację maszyny.

\begin{alltt}
rabbitmq.solutions.analyzers.number=\textcolor{OliveGreen}{2}
\end{alltt}

\noindent W obu przypadkach ustawienia są wczytywane przy starcie aplikacji.

\smallskip

RabbitMQ jest narzędziem sprawdzonym i powszechnie wykorzystywanym. To dlatego znalazł zastosowanie w systemie weryfikacji pomimo, że 
możliwe było również zaimplementowanie podobnego rozwiązania przy pomocy Akki. 

\subsection{AngularJS}

Interfejs aplikacji zarządzającej składa się jedynie z dwóch stron. Pierwsza z nich to strona logowania. Zbudowana została przy pomocy metod do tworzenia formularzy w widokach dostarczanych przez Play Framework.

Obsługa interfejsu ,,strony głównej'' zorganizowana została z wykorzystaniem biblioteki AngularJS. Jej głównym przeznaczeniem jest wsparcie dla szybkiego budowania dynamicznych aplikacji internetowych. Cel ten udało się osiągnąć dzięki dwukierunkowemu wiązaniu danych oraz stworzeniu modułu nazywanego ,,kompilatorem HTML''.

Dwukierunkowe wiązanie danych powoduje, że wszelkie zmiany w widoku są odzwierciedlane w modelu danych i na odwrót. Za dostarczenie odpowiednich informacji do konkretnego fragmentu strony odpowiada za tzw. \emph{kontroler}. Tworzy on i modyfikuje dane, które są przechowywane w specjalnym obiekcie \texttt{\$scope}. To dzięki niemu realizowane jest dwukierunkowe wiązanie danych. Modyfikowanie danych przechowywanych w \texttt{\$scope} sprawia, że zmiany są automatycznie zauważalne w widoku, bez potrzeby samodzielnego manipulowania elementami strony. 

Moduł ,,kompilatora HTML'', pozwala programiście zdefiniować kod strony w~zupełnie nowy sposób. Istnieje możliwość tworzenia własnych atrybutów, a~nawet znaczników. W ten sposób można dodać specyficzne dla potrzeb aplikacji ,,zachowanie'' poszczególnych elementów strony. Programista dokonuje tego za pomocą definiowania własnych dyrektyw. Część zarządzająca omawianego systemu potrzebowała dyrektyw decydujących o tym, które z elementów strony powinny być widoczne, bądź ukryte w zależności od roli zalogowanego użytkownika. Dzięki temu modułowi udało się stworzyć czytelne dla każdego atrybuty HTML.

\begin{minted}[]{html}
<div data-has-permission="Educator" class="pull-right">
...
</div>
\end{minted}

Fragment strony zawarty pomiędzy znacznikami \texttt{div} w powyższym przykładzie będzie pokazany jedynie prowadzącemu. Jego rola w kodzie systemu określona jest jako \texttt{Educator} (ang. dydaktyk). Stworzona dyrektywa pozwala zdefiniować wyświetlaną zawartość dzięki dodaniu czytelnego atrybutu \texttt{data-has-permission}.

Sama biblioteka AngularJS, jak i stworzona z jej pomocą aplikacja zarządzająca mają budowę modularną. Każdy model w ramach aplikacji posiada też własny, specyficzny zestaw serwisów i kontrolerów.

% zakończenie 
\summary
Scala to język warty szczególnej uwagi. Oferowane przez nią połączenie paradygmatu programowania obiektowego~i~funkcyjnego oraz kompatybilność z językiem Java stwarzają ogromne możliwości. W wielu miejscach już teraz Scala mogłaby swobodnie zastąpić Javę. System weryfikacji jakości kodu pozwala przy użyciu dojrzałych narzędzi testowania nie tylko zweryfikować kod, ale również przyczynić się do wzrostu popularności języka Scala. Zawiera w sobie element dydaktyczny, pozwalający na definiowanie własnych kursów i~zadań. Użyty w ośrodkach edukacyjnych, ma szansę przyczynić się do wzrostu zainteresowania programowaniem~i~podniesienia umiejętności przyszłych programistów. Z powodzeniem może też być wykorzystywany w firmach chcących sprawdzić kwalifikacje kandydatów~w~procesie rekrutacyjnym. System powstał przy użyciu wielu nowoczesnych technologii~i~jest dostosowany do pracy w architekturze rozproszonej.

\appendix
\chapter{Repozytoria aplikacji}

Poniżej znajdują się odsyłacze prowadzące do repozytoriów kodu aplikacji. W każdym z nich znajduje się krótka dokumentacja opisująca sposób ich konfiguracji~i~uruchamiania.

\begin{itemize}
\item Aplikacja zarządzająca -- \url{https://github.com/dsawa/scaxerciser}
\item Aplikacja testująca -- \url{https://github.com/dsawa/scaxerciser_analyze}
\item Szablon projektów SBT -- \url{https://github.com/dsawa/project-template}
\end{itemize}

Dostęp do repozytoriów jest ograniczony. Aby obejrzeć ich zawartość należy zalogować się na specjalnie utworzone konto:

\begin{itemize}
\item użytkownik -- \texttt{scaxerciser}
\item hasło -- \texttt{t3Gh1oBc}
\end{itemize}
