...
Obsługa interfejsu ,,strony głównej'' zorganizowana została z wykorzystaniem biblioteki AngularJS. Jej głównym przeznaczeniem jest wsparcie dla szybkiego budowania małych~i~dynamicznych aplikacji internetowych. Inżynierom Google'a cel ten udało się osiągnąć dzięki dwukierunkowemu wiązaniu danych oraz stworzeniu modułu nazywanego ,,kompilatorem HTML''.

Dwukierunkowe wiązanie danych w sposób automatyczny synchronizuje to, co jest wyświetlane na stronie, z tym co znajduje się w kontrolerze. Dzięki tej technice, wszelkie zmiany w widoku są odzwierciedlone~w~modelu danych i odwrotnie. Chociaż kontrolery są osobnymi bytami to możliwe jest modyfikowanie w nich danych bez potrzeby samodzielnego manipulowania elementami strony. Za poszczególne fragmenty strony odpowiadają osobne kontrolery JavaScript.
...
AngularJS sprawia, że aplikacja internetowa zbudowana jest z dostarczanych przez narzędzie modułów. Kod interfejsu aplikacji zarządzającej jest również modularny. Każdy model posiada własny, specyficzny zestaw serwisów i kontrolerów. Osobnymi modułami są również dyrektywy.
